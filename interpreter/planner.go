...
type interpretablePlanner interface {
	// PlanContext generates an Interpretable value (or error) from the input proto Expr.
	Plan(ctx context.Context, expr ast.Expr) (InterpretableContext, error)
}
...
func newPlanner(disp DispatcherContext,
	provider types.Provider,
	adapter types.Adapter,
	attrFactory AttributeFactoryContext,
	cont *containers.Container,
	exprAST *ast.AST,
	decorators ...InterpretableDecoratorContext) interpretablePlanner {
	return &planner{
		disp:        disp,
		provider:    provider,
		adapter:     adapter,
...
type planner struct {
	...
	decorators  []InterpretableDecoratorContext
}
...
func (p *planner) Plan(ctx context.Context, expr ast.Expr) (InterpretableContext, error) {
	switch expr.Kind() {
	case ast.CallKind:
		return p.decorate(p.planCall(ctx, expr))
	case ast.IdentKind:
		return p.decorate(p.planIdent(ctx, expr))
	case ast.LiteralKind:
		return p.decorate(p.planConst(ctx, expr))
	case ast.SelectKind:
		return p.decorate(p.planSelect(ctx, expr))
	case ast.ListKind:
		return p.decorate(p.planCreateList(ctx, expr))
	case ast.MapKind:
		return p.decorate(p.planCreateMap(ctx, expr))
	case ast.StructKind:
		return p.decorate(p.planCreateStruct(ctx, expr))
	case ast.ComprehensionKind:
		return p.decorate(p.planComprehension(ctx, expr))
	}
...
// for convenience.
func (p *planner) decorate(i InterpretableContext, err error) (InterpretableContext, error) {
	if err != nil {
		return nil, err
	}
...
func (p *planner) planIdent(ctx context.Context, expr ast.Expr) (InterpretableContext, error) {
	// Establish whether the identifier is in the reference map.
	if identRef, found := p.refMap[expr.ID()]; found {
		return p.planCheckedIdent(ctx, expr.ID(), identRef)
	}
	// Create the possible attribute list for the unresolved reference.
	ident := expr.AsIdent()
	return &evalAttr{
		adapter: p.adapter,
		attr:    p.attrFactory.MaybeAttributeContext(ctx, expr.ID(), ident),
	}, nil
}

func (p *planner) planCheckedIdent(ctx context.Context, id int64, identRef *ast.ReferenceInfo) (InterpretableContext, error) {
	// Plan a constant reference if this is the case for this simple identifier.
	if identRef.Value != nil {
		return NewConstValueContext(id, identRef.Value), nil
	}
	...
	if cType.Kind() == types.TypeKind {
		...
		return NewConstValueContext(id, cVal), nil
	}

	// Otherwise, return the attribute for the resolved identifier name.
	return &evalAttr{
		adapter: p.adapter,
		attr:    p.attrFactory.AbsoluteAttributeContext(ctx, id, identRef.Name),
	}, nil
}
...
func (p *planner) planSelect(ctx context.Context, expr ast.Expr) (InterpretableContext, error) {
	...
	attr, isAttr := op.(InterpretableAttributeContext)
	if !isAttr {
		attr, err = p.relativeAttr(ctx, op.ID(), op, false)
		if err != nil {
			return nil, err
		}
	}
	...
	if sel.IsTestOnly() {
		attr = &evalTestOnly{
			id:                            expr.ID(),
			InterpretableAttributeContext: attr,
		}
	}
	// Append the qualifier on the attribute.
	_, err = attr.AddQualifierContext(ctx, qual)
	return attr, err
}
...
func (p *planner) planCall(ctx context.Context, expr ast.Expr) (InterpretableContext, error) {
	call := expr.AsCall()
	...
	args := make([]InterpretableContext, argCount)
	if target != nil {
		arg, err := p.Plan(ctx, target)
		if err != nil {
			return nil, err
		}
		args[0] = arg
	...
	case operators.LogicalAnd:
		return p.planCallLogicalAnd(ctx, expr, args)
	case operators.LogicalOr:
		return p.planCallLogicalOr(ctx, expr, args)
	case operators.Conditional:
		return p.planCallConditional(ctx, expr, args)
	case operators.Equals:
		return p.planCallEqual(ctx, expr, args)
	case operators.NotEquals:
		return p.planCallNotEqual(ctx, expr, args)
	case operators.Index:
		return p.planCallIndex(ctx, expr, args, false)
	case operators.OptSelect, operators.OptIndex:
		return p.planCallIndex(ctx, expr, args, true)
	}

	// Otherwise, generate Interpretable calls specialized by argument count.
	// Try to find the specific function by overload id.
	var fnDef *functions.OverloadContext
	if oName != "" {
		fnDef, _ = p.disp.FindOverloadContext(oName)
	}
	// If the overload id couldn't resolve the function, try the simple function name.
	if fnDef == nil {
		fnDef, _ = p.disp.FindOverloadContext(fnName)
	}
	switch argCount {
	case 0:
		return p.planCallZero(ctx, expr, fnName, oName, fnDef)
	case 1:
		...
		if fnDef != nil && fnDef.Unary == nil && fnDef.Function != nil {
			return p.planCallVarArgs(ctx, expr, fnName, oName, fnDef, args)
		}
		return p.planCallUnary(ctx, expr, fnName, oName, fnDef, args)
	case 2:
		// If the FunctionOp has been used, then use it as it may exist for the purposes
		// of dynamic dispatch within a singleton function implementation.
		...
		return p.planCallBinary(ctx, expr, fnName, oName, fnDef, args)
	default:
		return p.planCallVarArgs(ctx, expr, fnName, oName, fnDef, args)
	}
}
...
func (p *planner) planCallZero(ctx context.Context, expr ast.Expr,
	function string,
	overload string,
	impl *functions.OverloadContext) (InterpretableContext, error) {
...
func (p *planner) planCallUnary(ctx context.Context, expr ast.Expr,
	function string,
	overload string,
	impl *functions.OverloadContext,
	args []InterpretableContext) (InterpretableContext, error) {
	var fn functions.UnaryOpContext
	var trait int
	var nonStrict bool
	if impl != nil {
		if impl.Unary == nil {
...
func (p *planner) planCallBinary(ctx context.Context, expr ast.Expr,
	function string,
	overload string,
	impl *functions.OverloadContext,
	args []InterpretableContext) (InterpretableContext, error) {
	var fn functions.BinaryOpContext
	var trait int
	var nonStrict bool
	if impl != nil {
		if impl.Binary == nil {
...
func (p *planner) planCallVarArgs(ctx context.Context, expr ast.Expr,
	function string,
	overload string,
	impl *functions.OverloadContext,
	args []InterpretableContext) (InterpretableContext, error) {
	var fn functions.FunctionOpContext
	var trait int
	var nonStrict bool
	if impl != nil {
...
func (p *planner) planCallEqual(ctx context.Context, expr ast.Expr, args []InterpretableContext) (InterpretableContext, error) {
	return &evalEq{
		id:  expr.ID(),
		lhs: args[0],
		rhs: args[1],
...
func (p *planner) planCallNotEqual(ctx context.Context, expr ast.Expr, args []InterpretableContext) (InterpretableContext, error) {
	return &evalNe{
		id:  expr.ID(),
		lhs: args[0],
		rhs: args[1],
...
func (p *planner) planCallLogicalAnd(ctx context.Context, expr ast.Expr, args []InterpretableContext) (InterpretableContext, error) {
	return &evalAnd{
		id:    expr.ID(),
		terms: args,
	}, nil
...
func (p *planner) planCallLogicalOr(ctx context.Context, expr ast.Expr, args []InterpretableContext) (InterpretableContext, error) {
	return &evalOr{
		id:    expr.ID(),
		terms: args,
	}, nil
...
func (p *planner) planCallConditional(ctx context.Context, expr ast.Expr, args []InterpretableContext) (InterpretableContext, error) {
	cond := args[0]
	t := args[1]
	var tAttr AttributeContext
	truthyAttr, isTruthyAttr := t.(InterpretableAttributeContext)
	if isTruthyAttr {
		tAttr = truthyAttr.AttrContext(ctx)
	} else {
		tAttr = p.attrFactory.RelativeAttributeContext(ctx, t.ID(), t)
	}

	f := args[2]
	var fAttr AttributeContext
	falsyAttr, isFalsyAttr := f.(InterpretableAttributeContext)
	if isFalsyAttr {
		fAttr = falsyAttr.AttrContext(ctx)
	} else {
		fAttr = p.attrFactory.RelativeAttributeContext(ctx, f.ID(), f)
	}

	return &evalAttr{
		adapter: p.adapter,
		attr:    p.attrFactory.ConditionalAttributeContext(ctx, expr.ID(), cond, tAttr, fAttr),
	}, nil
}
...
func (p *planner) planCallIndex(ctx context.Context, expr ast.Expr, args []InterpretableContext, optional bool) (InterpretableContext, error) {
	op := args[0]
	ind := args[1]
	...
	case InterpretableAttributeContext:
		qual, err = p.attrFactory.NewQualifier(opType, expr.ID(), ind, optional)
	default:
		qual, err = p.relativeAttr(ctx, expr.ID(), ind, optional)
	}
	if err != nil {
		return nil, err
	}
...
func (p *planner) planCreateList(ctx context.Context, expr ast.Expr) (InterpretableContext, error) {
	list := expr.AsList()
	...
	elems := make([]InterpretableContext, len(elements))
	for i, elem := range elements {
		elemVal, err := p.Plan(ctx, elem)
		if err != nil {
			return nil, err
		}
		elems[i] = elemVal
...
func (p *planner) planCreateMap(ctx context.Context, expr ast.Expr) (InterpretableContext, error) {
	m := expr.AsMap()
	entries := m.Entries()
	optionals := make([]bool, len(entries))
	keys := make([]InterpretableContext, len(entries))
	vals := make([]InterpretableContext, len(entries))
	for i, e := range entries {
		entry := e.AsMapEntry()
		keyVal, err := p.Plan(ctx, entry.Key())
		if err != nil {
			return nil, err
		}
		keys[i] = keyVal

		valVal, err := p.Plan(ctx, entry.Value())
		if err != nil {
			return nil, err
		}
		vals[i] = valVal
...
func (p *planner) planCreateStruct(ctx context.Context, expr ast.Expr) (InterpretableContext, error) {
	obj := expr.AsStruct()
	...
	vals := make([]InterpretableContext, len(objFields))
	for i, f := range objFields {
		field := f.AsStructField()
		fields[i] = field.Name()
		val, err := p.Plan(ctx, field.Value())
		if err != nil {
			return nil, err
		}
		vals[i] = val
...
func (p *planner) planComprehension(ctx context.Context, expr ast.Expr) (InterpretableContext, error) {
	fold := expr.AsComprehension()
	accu, err := p.Plan(ctx, fold.AccuInit())
	if err != nil {
		return nil, err
	}
	iterRange, err := p.Plan(ctx, fold.IterRange())
	if err != nil {
		return nil, err
	}
	cond, err := p.Plan(ctx, fold.LoopCondition())
	if err != nil {
		return nil, err
	}
	step, err := p.Plan(ctx, fold.LoopStep())
	if err != nil {
		return nil, err
	}
	result, err := p.Plan(ctx, fold.Result())
	if err != nil {
		return nil, err
	}
	return &evalFold{
...
func (p *planner) planConst(ctx context.Context, expr ast.Expr) (InterpretableContext, error) {
	return NewConstValueContext(expr.ID(), expr.AsLiteral()), nil
}
...
func (p *planner) relativeAttr(ctx context.Context, id int64, eval InterpretableContext, opt bool) (InterpretableAttributeContext, error) {
	eAttr, ok := eval.(InterpretableAttributeContext)
	if !ok {
		eAttr = &evalAttr{
			adapter:  p.adapter,
			attr:     p.attrFactory.RelativeAttributeContext(ctx, id, eval),
			optional: opt,
		}
	}
	...
	eAttr, ok = decAttr.(InterpretableAttributeContext)
	if !ok {
