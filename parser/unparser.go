...
func (un *unparser) visitCallBinary(expr *exprpb.Expr) error {
	...
	args := c.GetArgs()
	lhs := args[0]
	lhsParen := isHigherPrecedence(fun, lhs)
	rhs := args[1]
	rhsParen := isHigherPrecedence(fun, rhs)
...
func isSamePrecedence(op string, expr *exprpb.Expr) bool {
	...
	c := expr.GetCallExpr()
	other := c.GetFunction()
	return operators.Precedence(other) == operators.Precedence(op)
}

func isHigherPrecedence(op string, expr *exprpb.Expr) bool {
	if expr.GetCallExpr() == nil {
		return false
	}
	c := expr.GetCallExpr()
