...
func TestTimestamp_ReceiveGetHours(t *testing.T) {
	...
	ts := Timestamp{&timestamp.Timestamp{Seconds: 7506}}
	hr := ts.Receive(overloads.TimeGetHours, overloads.TimestampToHours, []refpb.Value{})
...
func TestTimestamp_ReceiveGetMinutes(t *testing.T) {
	// 1970-01-01T02:05:05Z
	ts := Timestamp{&timestamp.Timestamp{Seconds: 7506}}
	min := ts.Receive(overloads.TimeGetMinutes, overloads.TimestampToMinutes, []refpb.Value{})
...
func TestTimestamp_ReceiveGetSeconds(t *testing.T) {
	// 1970-01-01T02:05:05Z
	ts := Timestamp{&timestamp.Timestamp{Seconds: 7506}}
	sec := ts.Receive(overloads.TimeGetSeconds, overloads.TimestampToSeconds, []refpb.Value{})
